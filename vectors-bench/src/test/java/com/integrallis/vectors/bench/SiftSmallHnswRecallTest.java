...
/**
 * Regression guards for HNSW recall on the SIFT Small dataset (10K base, 100 queries, 128 dims,
 * L2). Tests multiple ef (beam width) values to anchor the recall–QPS tradeoff curve. Skipped when
 * SIFT Small files are absent.
 *
...
class SiftSmallHnswRecallTest {
  ...
  // -------------------------------------------------------------------------
  // Full-precision: recall floors at increasing ef (beam width)
  // -------------------------------------------------------------------------

  @Test
  void fullPrecision_ef64_recall10_above90() {
    double recall = meanRecall(10, 64);
    assertThat(recall)
        .as("HNSW full-precision ef=64 recall@10 on SIFT Small")
        .isGreaterThanOrEqualTo(0.90);
  }

  @Test
  void fullPrecision_ef128_recall10_above95() {
    double recall = meanRecall(10, 128);
    assertThat(recall)
        .as("HNSW full-precision ef=128 recall@10 on SIFT Small")
        .isGreaterThanOrEqualTo(0.95);
  }

  @Test
  void fullPrecision_ef200_recall10_above98() {
    double recall = meanRecall(10, 200);
    assertThat(recall)
        .as("HNSW full-precision ef=200 recall@10 on SIFT Small")
  ...
  @Test
  void twoPassSq8_ef128_recall10_above93() {
    ...
    try {
      ...
      assertThat(recall)
          .as("HNSW SQ8 two-pass ef=128 overQuery=2x recall@10 on SIFT Small")
  ...
  private static double meanRecall(int k, int efSearch) {
    int[][] approx = new int[queries.length][];
    for (int i = 0; i < queries.length; i++) {
      SearchResult r = index.search(queries[i], k, efSearch);
      approx[i] = r.nodeIds();
    }
    return RecallUtil.meanRecallAtK(groundTruth, approx, k);
  }

  private static double meanRecallTwoPass(int k, int efSearch, float overQueryFactor) {
    int[][] approx = new int[queries.length][];
    for (int i = 0; i < queries.length; i++) {
      SearchResult r = index.searchTwoPass(queries[i], k, efSearch, overQueryFactor);
      approx[i] = r.nodeIds();
    }
    return RecallUtil.meanRecallAtK(groundTruth, approx, k);
  }
