...
| 2026-09-04/05 | Which Q8 execution-layout and scheduling changes survive Soprano's complete speech-synthesis gate? | [`soprano-q8-experiments-20260905.md`](soprano-q8-experiments-20260905.md) connects the retained Vectors reproducers to the model-level evidence. Prepared F32 vocoder weights improved the decisive projection by 4.01x; split Q8 storage, two-dimensional tiling, and a lower parallel threshold were rejected. | Keep `GgufQ8_0F32Matrix` for the measured memory/throughput trade. Preserve the rejected benchmark-only candidates, keep the production Q8 threshold, and require future packed-dot work to clear waveform and streaming gates rather than only JMH. |
| 2026-09-06 | Can DeBERTa reranker F16 weights execute directly from mapped Safetensors without paying an F32 expansion? | [`mapped-float16-reranker-20260906.md`](mapped-float16-reranker-20260906.md) records a same-value comparison at all three xsmall projection shapes and batches 1, 32, and 128. Scalar mapped F16 was 5.59x-8.85x slower; the Vector API decode candidate was 12.50x-49.11x slower than expanded F32. | Reject mapped F16 as a production primitive. Retain the benchmark-only reproducer, expand this compact reranker's immutable weights once, and carry direct vector binary16 conversion forward as a JVM request. |
| 2026-09-14 | Can small F32 LoRA matrices remain bit-identical before and after HotSpot compilation without a warmup or native kernel? | [`f32-adapter-determinism-20260914.md`](f32-adapter-determinism-20260914.md) isolates an unspecified `reduceLanes(ADD)` tree, adds fresh-JVM 64/128/256-bit controls, and measures an owned fixed-tree layout at the real `32x2048` and `2048x32` shapes. The combined pair improved 25.49% on the tested Intel host. | Retain the owned deterministic F32 execution path for immutable adapter weights, leave the general mapped kernel unchanged, remove Models prewarming after adoption, and carry an explicitly ordered Vector API reduction forward as a JVM request. |
| 2026-09-16 | Does a dequantise-to-F32 panel + FMA register-tile batched matmul (idea from qxotic JAM `BandGemm`) beat the integer Q4_K/Q6_K/Q8_0 kernels? | [`2026-09-16-band-gemm/README.md`](2026-09-16-band-gemm/README.md) records a pre-registered rule and an AVX2-only screen on a heavily loaded i7-9750H: band 5.2x-10.4x faster at Granite 4.1 3B FFN prefill (batch 512), 0.32x-0.78x at decode; dequantization dominates decode. Tolerance tests pass (5.0e-8 of sum abs(w x)). | Rule not met on AVX2; keep as an explicit, default-off experiment (`vectors.gguf.batchedMatmulKernel=band`). AVX-512 and NEON runs pending; batch-size dispatch and vectorized dequant are follow-ups. |
